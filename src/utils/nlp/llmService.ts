 ...
 * LLM提供商类型
 */
export type LLMProvider = 'openai' | 'claude' | 'deepseek' | 'siliconflow';
...
export class LLMService {
    ...
    private async callAPI(input: string, htmlContext: string): Promise<string> {
        switch (this.config.provider) {
            ...
            case 'deepseek':
                return this.callDeepSeek(input, htmlContext);
            case 'siliconflow':
                return this.callSiliconFlow(input, htmlContext);
            default:
...
5. 对于复杂的选择器和伪类/伪元素，务必使用 "style" 方法
     ...
     * @returns API响应内容
     */
    private async callDeepSeek(input: string, htmlContext: string): Promise<string> {
        if (!this.config.apiKey) {
...
5. 对于复杂的选择器和伪类/伪元素，务必使用 "style" 方法
        ...
        if (!response.ok) {
            console.error('DeepSeek API request failed:', response);
     ...
     * @returns API响应内容
     */
    private async callSiliconFlow(input: string, htmlContext: string): Promise<string> {
        if (!this.config.apiKey) {
...
5. 对于复杂的选择器和伪类/伪元素，务必使用 "style" 方法
        ...
        if (!response.ok) {
            console.error('SiliconFlow API request failed:', response);
            throw new Error(`SiliconFlow API request failed: ${response.statusText}`);
        }
