...
12. 重要：对于需要清理的脚本（如动画、定时器、全局事件监听），必须实现自清理逻辑。使用特殊的占位符 {{{SCRIPT_ID}}} 代表脚本自身的ID。脚本必须使用document监听一个名为 'cleanup-{{{SCRIPT_ID}}}' 的自定义事件，并在收到该事件时，停止所有动画、移除所有自行创建的DOM元素、全局事件监听器。
    `.trim();
  }
